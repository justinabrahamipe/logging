...
export default function EditCycleGoalPage() {
  ...
  }, [session, status, router, cycleId, goalId]);

  const handleSubmit = async () => {
    if (!form.name.trim() || !form.targetValue || !form.unit.trim()) return;
    setSaving(true);
    try {
      const res = await fetch(`/api/cycles/${cycleId}/goals/${goalId}`, {
        method: "PUT",
  ...
  return (
    <div className="container mx-auto px-4 py-4 md:py-8 max-w-4xl">
      <div className="flex items-center gap-3 mb-6">
        ...
        <h1 className="text-2xl md:text-3xl font-bold text-gray-900 dark:text-white">Edit Goal</h1>
      </div>
